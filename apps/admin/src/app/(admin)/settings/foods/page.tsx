...
export default async function FoodsPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  const query = (
    ...
    .from('food_db')
    .select(
      'id, name_pt, category, kcal_per_100g, protein_g, carbs_g, fat_g, fiber_g, country_code, source, source_ref, is_verified',
      { count: 'exact' },
    )
  ...
  let q = query as unknown as {
    ...
    eq: (col: string, val: string | boolean) => typeof q
  ...
  if (params.verified) q = q.eq('is_verified', params.verified === 'true')
      ...
      <PageHeader
        ...
        description={
          <>
            ...
            cadastrados. Referências manuais ficam em quarentena até receberem uma fonte auditável.
          </>
        }
      />

      <FoodsTable rows={(data ?? []) as FoodRow[]} categories={categories} searchParams={params} />
    </div>
  )
}
