...
if [[ "$EXTERNAL_AUTHORITY_LAUNCHER" == '1' ]]; then
  ...
  for EXTERNAL_REQUIRED_PATH in \
    "$EXTERNAL_AUTHORITY_PATH" "$EXTERNAL_CONTROLLER_PATH" \
  ...
  [[ "$EXTERNAL_AUTHORITY_LINES[1]" == 'CI3_EXTERNAL_LAUNCHER_AUTHORITY_V1'
     || "$EXTERNAL_AUTHORITY_LINES[1]" == 'CI3_EXTERNAL_LAUNCHER_AUTHORITY_V2' ]] || fail STOP_PRE_AUTHORITY
  EXTERNAL_CAPSULE_BOUND=0
  if [[ "$EXTERNAL_AUTHORITY_LINES[1]" == 'CI3_EXTERNAL_LAUNCHER_AUTHORITY_V2' ]]; then
    EXTERNAL_CAPSULE_BOUND=1
    (( ${#EXTERNAL_AUTHORITY_LINES} == 13 )) || fail STOP_PRE_AUTHORITY
  else
  ...
  if [[ "$EXTERNAL_CAPSULE_BOUND" == '1' ]]; then
    [[ "$EXTERNAL_AUTHORITY_LINES[9]" == 'node_capsule_manifest_sha256 '*
       && "$EXTERNAL_AUTHORITY_LINES[10]" == 'node_capsule_receipt_sha256 '*
       && "$EXTERNAL_AUTHORITY_LINES[11]" == 'production_frozen_inputs_sha256 '*
       && "$EXTERNAL_AUTHORITY_LINES[12]" == 'allowed_modes '*
       && "$EXTERNAL_AUTHORITY_LINES[13]" == 'raw_values false' ]] || fail STOP_PRE_AUTHORITY
    EXTERNAL_CAPSULE_MANIFEST_SHA="${EXTERNAL_AUTHORITY_LINES[9]#node_capsule_manifest_sha256 }"
    EXTERNAL_CAPSULE_RECEIPT_SHA="${EXTERNAL_AUTHORITY_LINES[10]#node_capsule_receipt_sha256 }"
    EXTERNAL_PRODUCTION_INPUTS_SHA="${EXTERNAL_AUTHORITY_LINES[11]#production_frozen_inputs_sha256 }"
    [[ "$EXTERNAL_CAPSULE_MANIFEST_SHA" =~ '^[0-9a-f]{64}$'
       && "$EXTERNAL_CAPSULE_RECEIPT_SHA" =~ '^[0-9a-f]{64}$'
       && "$EXTERNAL_PRODUCTION_INPUTS_SHA" =~ '^[0-9a-f]{64}$' ]] || fail STOP_PRE_AUTHORITY
    EXTERNAL_NODE_CAPSULE_ROOT="$EXTERNAL_RUNTIME_ROOT/node-capsule"
    EXTERNAL_NODE_PATH="$EXTERNAL_NODE_CAPSULE_ROOT/capsule/bin/node"
    EXTERNAL_CAPSULE_MANIFEST_PATH="$EXTERNAL_NODE_CAPSULE_ROOT/capsule-manifest.json"
    EXTERNAL_CAPSULE_RECEIPT_PATH="$EXTERNAL_NODE_CAPSULE_ROOT/mac-relocatable-node-capsule.receipt.json"
    EXTERNAL_ALLOWED_MODES="${EXTERNAL_AUTHORITY_LINES[12]#allowed_modes }"
    for EXTERNAL_REQUIRED_PATH in "$EXTERNAL_NODE_PATH" "$EXTERNAL_CAPSULE_MANIFEST_PATH" "$EXTERNAL_CAPSULE_RECEIPT_PATH"; do
      [[ -f "$EXTERNAL_REQUIRED_PATH" && ! -L "$EXTERNAL_REQUIRED_PATH" ]] || fail STOP_PRE_AUTHORITY
    done
  else
  ...
  if [[ "$EXTERNAL_CAPSULE_BOUND" == '1' ]]; then
    [[ "$(external_hash "$EXTERNAL_CAPSULE_MANIFEST_PATH")" == "$EXTERNAL_CAPSULE_MANIFEST_SHA"
       && "$(external_hash "$EXTERNAL_CAPSULE_RECEIPT_PATH")" == "$EXTERNAL_CAPSULE_RECEIPT_SHA"
       && "$(external_hash "$EXTERNAL_ATTESTATION_PATH")" == "$EXTERNAL_ATTESTATION_SHA" ]] || fail NODE_IDENTITY
    /usr/bin/grep -Fq '"purpose":"CI3_GIT_BOUND_LAUNCH_ATTESTATION_V3"' "$EXTERNAL_ATTESTATION_PATH" || fail NODE_IDENTITY
    /usr/bin/grep -Fq "\"production_frozen_inputs\"" "$EXTERNAL_ATTESTATION_PATH" || fail NODE_IDENTITY
  fi
