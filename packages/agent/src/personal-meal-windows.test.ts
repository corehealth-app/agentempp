...
describe('buildPersonalWindowsFromLogs — Roberto janta 17-19h ET', () => {
  it('com 5+ amostras de jantar 17-19h, gera janela pessoal 17-19', () => {
    const logs: Array<{ meal_type: MealType; hour: number }> = [
      { meal_type: 'jantar', hour: 17 },
      { meal_type: 'jantar', hour: 18 },
      { meal_type: 'jantar', hour: 19 },
      { meal_type: 'jantar', hour: 18 },
      { meal_type: 'jantar', hour: 17 },
      { meal_type: 'jantar', hour: 19 },
    ]
  ...
  it('grupos diferentes — só ativa quem tem ≥5', () => {
    const logs: Array<{ meal_type: MealType; hour: number }> = [
      // 5x cafe
      { meal_type: 'cafe', hour: 7 },
      { meal_type: 'cafe', hour: 8 },
      { meal_type: 'cafe', hour: 9 },
      { meal_type: 'cafe', hour: 8 },
      { meal_type: 'cafe', hour: 7 },
      // 2x almoco (insuficiente)
      { meal_type: 'almoco', hour: 13 },
      { meal_type: 'almoco', hour: 14 },
    ]
    const windows = buildPersonalWindowsFromLogs(logs)
  ...
  it('ceia com wrap noturno (23,0,1,2,23,0) — janela cruza meia-noite', () => {
    const logs: Array<{ meal_type: MealType; hour: number }> = [
      { meal_type: 'ceia', hour: 23 },
      { meal_type: 'ceia', hour: 0 },
      { meal_type: 'ceia', hour: 1 },
      { meal_type: 'ceia', hour: 2 },
      { meal_type: 'ceia', hour: 23 },
      { meal_type: 'ceia', hour: 0 },
    ]
    const windows = buildPersonalWindowsFromLogs(logs)
...
describe('resolveMealTypeByHour — fallback global', () => {
  ...
  it('sem janela pessoal, 2h → lanche/madrugada (global preserva backward-compat)', () => {
    expect(resolveMealTypeByHour(2, emptyPersonal).expected).toBe('lanche')
  })
})
...
describe('resolveMealTypeByHour — Roberto ET (janta 17-19h)', () => {
  it('com janela pessoal jantar=17-19, 17h NÃO vira lanche, vira jantar', () => {
    const personal: PersonalMealWindows = {
      windows: new Map([
        ['jantar' as MealType, { meal_type: 'jantar' as MealType, p10: 17, p90: 19, sample_count: 8 }],
      ]),
      totalLogs: 30,
    }
  ...
  it('hora fora da janela pessoal → cai pra global', () => {
    ...
    const personal: PersonalMealWindows = {
      windows: new Map([
        ['cafe' as MealType, { meal_type: 'cafe' as MealType, p10: 7, p90: 9, sample_count: 10 }],
      ]),
      totalLogs: 30,
    }
  ...
  it('sobreposição: janelas almoco 13-15 + lanche 14-16, hora 14 → escolhe a mais estreita', () => {
    ...
    const personal: PersonalMealWindows = {
      windows: new Map([
        ['almoco' as MealType, { meal_type: 'almoco' as MealType, p10: 13, p90: 15, sample_count: 6 }],
        ['lanche' as MealType, { meal_type: 'lanche' as MealType, p10: 14, p90: 16, sample_count: 6 }],
      ]),
      totalLogs: 30,
    }
  ...
  it('span menor ganha: almoco 12-15 (span=3) vs lanche 14-15 (span=1), hora 14 → lanche', () => {
    const personal: PersonalMealWindows = {
      windows: new Map([
        ['almoco' as MealType, { meal_type: 'almoco' as MealType, p10: 12, p90: 15, sample_count: 6 }],
        ['lanche' as MealType, { meal_type: 'lanche' as MealType, p10: 14, p90: 15, sample_count: 6 }],
      ]),
      totalLogs: 30,
    }
    const r = resolveMealTypeByHour(14, personal)
  ...
  it('ceia com wrap (p10=23, p90=2), hora 0 → ceia personal', () => {
    const personal: PersonalMealWindows = {
      windows: new Map([
        ['ceia' as MealType, { meal_type: 'ceia' as MealType, p10: 23, p90: 2, sample_count: 6 }],
      ]),
      totalLogs: 30,
    }
