...
private func validateMacCapsuleInstallTopology(_ entries: [Entry], handoff: [String: Any]) throws {
    ...
    let node = entries[3]
    guard try integer(manifest["schema_version"]) == 4,
          try string(manifest["purpose"]) == "MAC_RELOCATABLE_NODE_CAPSULE_V4",
          try string(manifest["authority"]) == string(handoff["authority_sha"]),
          try string(manifest["generation"]) == "capsule-v4",
          try string(manifest["role"]) == "MAC_EXECUTOR_NODE_RUNTIME",
          try string(manifest["predecessor_authority"]) == "85a9ebba88722915df56583d29defc253016a5f9",
          try string(manifest["predecessor_generation"]) == "capsule-v3",
    ...
    try exactKeys(receipt, [
        ...
        "capsule_executable_sha256", "capsule_images_sha256", "ready_handshakes", "stable_observations",
        "independent_source_observations", "mandatory_load_set_complete", "weak_lazy_policy", "copied_but_unused",
    ])
    guard try integer(receipt["schema_version"]) == 4,
          try string(receipt["purpose"]) == "MAC_RELOCATABLE_NODE_CAPSULE_V4",
          try string(receipt["authority"]) == string(handoff["authority_sha"]),
          try string(receipt["generation"]) == "capsule-v4",
          ...
          try string(receipt["ready_handshakes"]) == "2/2_PASS",
          try string(receipt["stable_observations"]) == "4/4_PASS",
          try string(receipt["independent_source_observations"]) == "2/2_PASS",
          try boolean(receipt["mandatory_load_set_complete"]) == true,
          try string(receipt["weak_lazy_policy"]) == "PASS",
          try integer(receipt["copied_but_unused"]) == 0,
